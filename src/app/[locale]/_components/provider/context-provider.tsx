...
export function ContextProvider({ t, playlistOption, pageParams, children }: ProviderProps) {
  ...
  const setOptions = useCallback(
    (option: Partial<PlaylistOption>) => {
      ...
      localStorage.setItem('ambient.options', JSON.stringify({ ...options, ...option }));
  ...
  useEffect(() => {
    ...
    if (storedOptionsString) {
      const storedOptions = (JSON.parse(storedOptionsString) as PlaylistOption) ?? {};
      ...
      localStorage.setItem(
        'ambient.options',
        JSON.stringify({ ...defaultPlaylistOption, ...playlistOption, ...storedOptions })
      );
  ...
  return (
    <TranslationContext value={t}>
      <PageParamsContext value={pageParams}>
        ...
        >
          <DrawerContext value={{ playlistOpen, setPlaylistOpen, settingsOpen, setSettingsOpen }}>
            <ModalContext value={{ modalOpen, setModalOpen }}>{children}</ModalContext>
          </DrawerContext>
        </PlayerContext>
      </PageParamsContext>
    </TranslationContext>
