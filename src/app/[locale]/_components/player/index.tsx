...
export async function Player({ playlist, mediaList }: PlayerProps) {
  ...
  const caption =
    media && playlist?.options.caption ? filterText(media, playlist.options.caption) : '';
  ...
  return (
    ...
    >
      <div className='relative w-full mt-4 flex justify-center'>
        <PrevButton mediaCount={mediaList.length} disabled={mediaList.length < 2} />
        <div className='relative h-56 max-w-sm w-96 overflow-hidden rounded-lg md:h-64'>
          <Thumbnail media={media} />
        </div>
        <NextButton mediaCount={mediaList.length} disabled={mediaList.length < 2} />
      </div>
