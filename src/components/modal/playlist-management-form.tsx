...
export default function PlaylistManagementForm({ t }: PlaylistManagementFormProps) {
  const {
    register,
    watch,
    trigger,
  ...
  const onSubmit = (data: PlaylistManagementFormValues) => {
    console.log(data); // TODO: Implement this feature
  };

  const download = () => {
    console.log('Download playlist'); // TODO: Implement this feature
  };
        ...
        <div id='playlist-management-field-symbolic-link' className='mb-4'>
          ...
          <Label
            ...
            className={cn(
              ...
              errors.localMediaDir && isLocal && 'text-red-600 dark:text-red-400',
              !errors.localMediaDir &&
                !!watch('localMediaDir') &&
                isLocal &&
                'text-green-500 dark:text-green-400'
            )}
            touched={!!watch('localMediaDir')}
            required={isLocal}
            error={Boolean(errors.localMediaDir) ? t['This path is required'] : ''}
            t={t}
          >
          ...
          <input
            ...
            required
            disabled={!isLocal}
            {...register('localMediaDir', { onBlur: () => trigger('localMediaDir') })}
          />
        ...
        <div className='grid gap-4 mb-8 md:grid-cols-2'>
          <div>
            <Label
              ...
              className={cn(
                ...
                !errors.symlinkName && isLocal
                  ? 'text-gray-900 dark:text-white'
                  : 'text-gray-600 dark:text-gray-400',
                errors.symlinkName && isLocal && 'text-red-600 dark:text-red-400',
                !errors.symlinkName &&
                  !!watch('symlinkName') &&
                  isLocal &&
                  'text-green-500 dark:text-green-400'
              )}
              touched={!!watch('symlinkName')}
              required={isLocal}
              error={Boolean(errors.symlinkName) ? t['This name is required'] : ''}
              t={t}
            >
            ...
            <input
              ...
              required
              disabled={!isLocal}
              {...register('symlinkName', { onBlur: () => trigger('symlinkName') })}
            />
          </div>
          <div className='flex justify-end items-end'>
            <button
              ...
              disabled={!isLocal || Boolean(errors.localMediaDir) || Boolean(errors.symlinkName)}
            >
              {t['Create Symbolic Link']}
            </button>
          </div>
          ...
          <div className='grid gap-4 mb-4 md:grid-cols-2'>
            <div>
              <Label
                ...
                touched={!!watch('categoryName')}
                required
                error={Boolean(errors.categoryName) ? t['This name is required'] : ''}
                t={t}
              >
                {t['Category Name']}
              </Label>
