const init = function () {
  ...
  function updatePlaylist() {
    ...
    let items = [];
    if (
      !AMP_STATUS.hasOwnProperty('ctg') ||
      AMP_STATUS.ctg == null ||
      Number(AMP_STATUS.ctg) == -1
    ) {
    ...
    items.forEach((item) => {
      ...
      if ((item.image && item.image !== '') || (item.thumb && item.thumb !== '')) {
        if (AmbientData && AmbientData.imageDir) {
          imageSrc =
            AmbientData.imageDir + (item.thumb && item.thumb !== '' ? item.thumb : item.image);
        }
  ...
  function getOption(key) {
    if (AMP_STATUS.hasOwnProperty('options') && AMP_STATUS.options !== null) {
      if (
        !AMP_STATUS.options.hasOwnProperty(key) ||
        AMP_STATUS.options[key] === null ||
        AMP_STATUS.options[key] === ''
      ) {
        return null;
      } else {
  ...
  function clearCarousel() {
    ...
    $NO_MEDIA_IMAGE.setAttribute(
      'class',
      'absolute block h-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2'
    );
  ...
  function updateCarousel() {
    ...
    items.forEach((amId, index) => {
      ...
      if (mediaData.hasOwnProperty('image') && mediaData.image !== null && mediaData.image !== '') {
        ...
        mediaData.hasOwnProperty('videoid') &&
        mediaData.videoid !== null &&
        mediaData.videoid !== ''
      ) {
      ...
      $COROUSEL_ITEM_IMAGE.classList.add(
        'absolute',
        'block',
        base_aspect,
        '-translate-x-1/2',
      ...
      if (
        basename(mediaImage) === 'no-media-placeholder' &&
        isObject(AMP_STATUS.options) &&
        AMP_STATUS.options.hasOwnProperty('dark') &&
        AMP_STATUS.options.dark
      ) {
  ...
  function filterText(format, mediaData) {
    ...
    if (patterns.length > 0) {
      patterns.forEach((pattern) => {
        ...
        const replacer =
          mediaData.hasOwnProperty(property) && mediaData[property] ? mediaData[property] : '';
  ...
  function changeRangeVolume() {
    $RANGE_VOLUME.value = inRange(Number(AMP_STATUS.volume), 0, 100)
      ? Number(AMP_STATUS.volume)
      : 100;
  ...
  function playItem(object = null, id = null) {
    ...
    const amId =
      id !== null ? id : thisElm.dataset.playlistItem ? Number(thisElm.dataset.playlistItem) : 0;
    const mediaData = AMP_STATUS.media.filter((item) => item.amId == amId).shift();
    let mediaSrc = null;
    let playerType = null;
    ...
    playerElm.addEventListener('loadstart', (evt) => {
      ...
      setTimeout(() => {
        if (evt.target.readyState == 0) {
          logger(
            'warn',
            `The player will treat this media (${mediaData.file}) as unsupported and will skip it.`,
            'force'
          );
      ...
      if (self.tagName === 'VIDEO') {
        ...
        if (currentWindowSize.width >= 640) {
          ...
          self.height = Math.floor(
            ((currentWindowSize.width - 2) * self.videoHeight) / self.videoWidth
          );
        }
      }
    });
  ...
  function fadeIn(media, period, start) {
    ...
    fadeinId = setInterval(() => {
      const currentTime =
        (mediaType === 'youtube' ? media.getCurrentTime() : media.currentTime) * 1000; // unit milliseconds
  ...
  function updateNotice(notification) {
    ...
    const classes = {
      ...
      success:
        'text-green-800 border-green-300 bg-green-50 dark:text-green-400 dark:border-green-800 dark:bg-green-900',
      warning:
        'text-yellow-800 border-yellow-300 bg-yellow-50 dark:text-yellow-400 dark:border-yellow-800 dark:bg-yellow-900',
      error:
        'text-red-800 border-red-300 bg-red-50 dark:text-red-400 dark:border-red-800 dark:bg-red-900',
      btnbase:
        'ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 p-1.5 inline-flex items-center justify-center h-8 w-8 ',
      btninfo:
        'bg-blue-50 text-blue-500 focus:ring-blue-400 hover:bg-blue-200 dark:bg-blue-800 dark:text-blue-400 dark:hover:bg-blue-700',
      btnsuccess:
        'bg-green-50 text-green-500 focus:ring-green-400 hover:bg-green-200 dark:bg-green-800 dark:text-green-400 dark:hover:bg-green-700',
      btnwarning:
        'bg-yellow-50 text-yellow-500 focus:ring-yellow-400 hover:bg-yellow-200 dark:bg-yellow-800 dark:text-yellow-400 dark:hover:bg-yellow-700',
      btnerror:
        'bg-red-50 text-red-500 focus:ring-red-400 hover:bg-red-200 dark:bg-red-800 dark:text-red-400 dark:hover:bg-red-700',
    };
  ...
  watcher(
    ...
    (mutation) => {
      if (mutation.type === 'attributes' && mutation.attributeName === 'data-validate') {
        ...
        const contains = [
          mediaType === 'youtube' ? 'youtube-url' : 'local-media-file',
          'media-category',
          'media-title',
        ];
  ...
  watcher(
    ...
    (mutation) => {
      if (mutation.type === 'attributes' && mutation.attributeName === 'data-validate') {
        ...
        logger(
          `Check valid items for "Create Symlink":`,
          valid_items,
          symlink_contains,
          isSymlinkContainAll
        );
        ...
        logger(
          `Check valid items for "Create Category":`,
          valid_items,
          category_contains,
          isCategoryContainAll
        );
    ...
    if (targetElement) {
      ...
      if (result === null) {
        ...
        if (isElement($FIELD_LABEL))
          toggleClass($FIELD_LABEL, {
            'normal-text': true,
            'error-text': false,
            'success-text': false,
          });
        if (isElement($FIELD_PREFIX))
          toggleClass($FIELD_PREFIX, {
            'normal-prefix': true,
            'error-prefix': false,
        ...
        if (isElement($FIELD_PREFIX))
          toggleClass($FIELD_PREFIX, {
            'normal-prefix': !result,
            'error-prefix': !result,
            'success-prefix': result,
          });
...
function getOS() {
  let ua;
  return 'userAgentData' in window.navigator
    ? ((ua = navigator.userAgentData), ua.platform)
...
function getRect(targetElement, property = '') {
  if (isElement(targetElement)) {
    ...
    if ('' === property) return _RECT_OBJ;
    if (Object.getPrototypeOf(_RECT_OBJ).hasOwnProperty(property)) return _RECT_OBJ[property];
  }
  return !1;
}
...
function setStyles(targetElements, styles = '') {
  ...
  (targetElements instanceof Array ? targetElements : [targetElements]).map((elm) => {
    if (styles instanceof Object) for (const _prop in styles) elm.style[_prop] = styles[_prop];
    else elm.style.cssText = String(styles);
  });
}
...
function isHidden(targetElement, checkProperties) {
  if (!isElement(targetElement)) return !1;
  const elmStyles = window.getComputedStyle(targetElement),
    elmStatus = {};
  return (
    checkProperties
      ? checkProperties instanceof Array || (checkProperties = [checkProperties])
      : (checkProperties = ['display', 'opacity', 'visibility']),
    checkProperties.forEach((prop) => {
      elmStatus[prop] = elmStyles.getPropertyValue(prop);
    }),
    (elmStatus.hidden = getAtts(targetElement, 'hidden')),
    elmStatus.hidden ||
      'none' === elmStatus.display ||
      0 == Number(elmStatus.opacity) ||
      'hidden' === elmStatus.visibility
  );
}

function replaceTagName(targetElement, tagName) {
  if (!isElement(targetElement)) return targetElement;
...
function replaceAttribute(targetElement, attributeName, replacementName) {
  const attrValue = targetElement.getAttribute(attributeName),
    prevAttr = {};
  return (
    !(!isElement(targetElement) || !attrValue) &&
...
function watcher(targetElements, callback, config = {}) {
  ...
  if (!callback || 'function' != typeof callback) return !1;
  ...
  _ELMS.map((elm) => {
    if (!isElement(elm)) return logger('error', 'Watching target is not an HTML element.', !0), !1;
...
function loadStge(key) {
  ...
  try {
    const userData = JSON.parse(_data);
    if (isObject(userData) && userData.hasOwnProperty(key)) return userData[key];
  } catch (error) {
    logger(error, _data);
  }
  return null;
